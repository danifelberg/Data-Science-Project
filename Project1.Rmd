...
We can see the boxplot above of income brackets and total yearly electricity costs. On averge, lower incomes tend to spend less on electricity costs, compared to higher incomes. This is also supported in literature about energy poverty and cultural practices to save energy.
...
Having now answered some preliminary research questions through EDA, we now ask more complex questions that we will answer using a variety of models:

1.  **What factors impact the electricity costs related to heat pump implementation?**
2.  **What factors impact the total electricity costs of a US household, and how strong is that impact?**
...
## 4.2 Income, Urban Density, and Division

Looking now at our second question, we create some general linear models looking specifically at the influence that income, urban density, and census division have on electricity cost.
...
Above we see several models that we have created, and at the very bottom, we have an ANOVA test comparing these different models. The last row has the regression with the lowest F-score, and lowest sum of squares error, meaning that, among the three, it is the preferred regression:

**`Yearly Electricity Costs` \~ `Income` + `Urban Density` + `Division` + `Urban Density` \* `Division`**

Looking at the coefficient values of the model, we can observe that income has a positive impact on electricity costs, and in addition, households located in urban areas appear to have lower electricity costs than those in rural areas (when taking census divisions into account), contradicting what we observed in the EDA portion of this analysis. That being said, most of the interaction terms have a p-value greater than 0.05, meaning we cannot consider them to be statistically significant. Along with that, when looking at urban density alone, rural areas still appear to have much lower electricity costs than urban and urban cluster areas.

## 4.3 Division and Climate

Continuing with our second question, we now consider census division and climate by themselves, and generate some linear models.
...
The ANOVA comparison of Yearly Electricity Costs related to Climate and Division illustrate that the last row has the regression with the lowest F-score, and lowest sum of squares error, meaning that among the four, it is the preferred regression:

**`Yearly Electricity Costs`\~`Division` + `Climate` + `Division`\*`Climate`**

Interestingly, we can observe that humidity alone, without accounting for census divisions, appears to be impacting household electricity cost much more than the coefficients that contain interaction terms. Combining this with what we observed in the EDA portion, it may be the case that climate is impacting the cost much more than census division.

## 4.4 Stepwise Regression

Having a clearer idea of how some variables are specifically impacting household electricity costs, we now create a stepwise regression using a full model, that takes all variables we looked at so far:

**`Yearly_Electricity_Costs`\~`Income` + `Urban_Density`+ `Division` + `Climate` + `Total_Rooms` + `SqFoot` + `Urban_Density` \* `Division` + `Division` \* `Climate`**
...
With this more complete model, we confirm some patterns that we have observed so far. We see that income (generally), total number of rooms, and square footage have a positive correlation with electricity cost. Moreover, we once again notice that, when looking at urban density by itself (as many of the interaction terms are not significant), households in rural areas have higher electricity costs than those in urban and urban cluster areas. With that, humid climates also appear to have much higher electricity costs.

## 4.5 Regression Tree

To fully understand all the potential relationships of the variables on household energy expenditure, we created a regression tree that allows us to potentially predict how much a given household would spend on electricity. That being said, a regression tree by itself will not necessarily have accurate predictions, as they have a high variance. However, it still allows us to observe which variables have the most significant impact on electricity costs.
...
fancyRpartPlot(treefitRpart2, cex=0.75)
...
The regression tree shows yearly electricity costs being influenced by factors like total rooms, climate, square footage, and income. This is helpful for determining predictive factors for the variables. As we can see, households with more rooms, greater square footage, that resided in either extreme or humid climates, and had a higher income, tend to spend more on electricity, annually. Notably, division does not appear to have an impact, per the regression tree, showcasing once more that climate may influence energy costs much more than census division.

A plot showing the size vs. deviance was also created to show that the regression tree has a very low error. This would be concerning if we were looking to predict a specific cost based on the variables (i.e. overfitting), but since we are only looking at their importance, the tree will do just fine.

# 5 Conclusion

With the models that we created above, we now look back at the additional research questions we proposed in order to answer them.

1.  **What factors impact the electricity costs related to heat pump implementation?**

Based on the linear models, along with ridge regression, we found that heat pumps have decrease total energy expenditures by 1.4% of electricity costs per 1 square footage. This is observed when controlling for environmental and housing attributes, as well as fuel types.

1.  **What factors impact the total electricity costs of a US household, and how strong is that impact?**

Using the various linear models we created, along with the stepwise regression, we see that income, urban density, total rooms, area, and climate have a significant impact on electricity costs. At the same time, we also observed that division only has a mild impact compared to the other factors that were analyzed. One potential reason that this may be the case could be due to the fact that, certain census divisions are mostly made up by a single climate. Consequently, it is not so much that division is increasing household energy costs, but rather, the climate that is present in that division is mainly responsible for the increase.
